...
M.get(`accounts/${accountId}/statuses`, {limit: 40}) // ホームタイムラインの例
    .then(resp => {
        ...
        core.setOutput('contents_text', contentsText);
        ...
        // console.log(`saved to ${contentsOutputPath}`);

        // const rawString = JSON.stringify(timeline, null, 2);
        // const timestamp = new Date().toISOString().replace(/:/g, '-').replace(/\..+/, ''); // ミリ秒を除去
        // const rawFilename = `timeline-${timestamp}.json`;
        // core.setOutput('raw_string', rawString);
        ...
        // console.log(`saved to ${rawOutputPath}`);
    })
    .catch(err => {
